...
A relational database backend for micro-mobility platforms and bicycle rental shops. Engineered with automated constraint tracking, financial audit triggers, role-based database security authorization, and performance indexing.
...
- **Role-Based Access Control (RBAC)**: Configures isolated authorization parameters separating ManagerRole, StaffRole, and AnalystRole credentials.
...
3. `security.sql` (Provisions database level roles, user assignments, and schema access grants)
4. `seed_data.sql` (Populates development test data environments)
5. `verification.sql` (Runs analytics validation queries)
